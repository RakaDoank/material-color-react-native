...
ImageUtils.sourceHexColorFromImageUri(yourLocalImageUri) // second argument for the image processing options
  .then(sourceColor => {
...
ImageUtils.sourceHexColorFromImageUri(onlineImage) // second argument for the image processing options
  .then(sourceColor => {
    console.log(sourceColor)
    // sourceColor in hex code string
  })
