...
This library provide an option for you to build Material color with an image, either it's your static bundled image file, or a local image you get from a device file or photos or gallery, or an image from a network in React Native app.
...
export function ReactComponent() {
  ...
  useEffect(() => {
    ...
    MaterialColor.fromSourceImage(YourLocalImage)
      ...
      MaterialColor.fromSourceImageUri(localImageFile).then(materialColor => {
        // your logics
      })
...
From the static method, you will get the same [`MaterialColor`](/docs/definitions/index/classes/MaterialColor) instance. The difference is **the instance could be null**, or thrown an error if the process is timed out or aborted by your own [AbortSignal](https://https://www.google.com/search?client=firefox-b-d&q=AbortSignal).
...
ImageUtils.sourceColorFromImageUri(yourLocalImageUri) // second argument for maxWidthOrHeight, timeout, signal, other opts in the future
  .then(sourceColor => {
...
ImageUtils.sourceColorFromImageUri(onlineImage) // second argument for maxWidthOrHeight, timeout, signal. (other opts in the future)
  .then(sourceColor => {
    console.log(sourceColor)
    // sourceColor in hex code string
  })
...
export function YourReactComponent() {
  ...
  const { data, isLoading, error } = useMaterialColorFromSourceImage(YourLocalImage)
...
### useMaterialColorFromSourceImageUri
...
export function YourReactComponent() {
  ...
  const { data, isLoading, error } = useMaterialColorFromSourceImageUri(onlineImage)
...
- [useMaterialColorFromSourceImage](/docs/definitions/index/functions/useMaterialColorFromSourceImage)
- [useMaterialColorFromSourceImageUri](/docs/definitions/index/functions/useMaterialColorFromImageUri)
