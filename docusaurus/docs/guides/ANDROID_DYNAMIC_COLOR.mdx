...
`AndroidDynamicColor` provides an instance as similar as possible to the [`MaterialColor`](/docs/material-color), so you can easily access and seamless integration. The main difference is that we can't get the source color from Android Dynamic Color, it's simply because no Android API provides it.

Then, if Android Dynamic Color is not available in user device, you can use [`MaterialColor`](/docs/material-color) as a fallback if it does.
...
As an alternative, and probably better, you can use the [`useAndroidDynamicColorCompat`](#useandroiddynamiccolorcompat)

You can also see full example here at [this project repository](https://github.com/RakaDoank/material-color-react-native)
...
Alternatively, you can use [`useAndroidDynamicColorCompat`](/docs/definitions/index/functions/useAndroidDynamicColorCompat) in your React component to get Material color for non Android platform and older Android SDK by giving a source color to build [`MaterialColor`](/docs/material-color) as fallback if it does
...
- [AndroidDynamicColor](/docs/definitions/index/classes/AndroidDynamicColor)
- [useAndroidDynamicColor](/docs/definitions/index/functions/useAndroidDynamicColor)
- [useAndroidDynamicColorCompat](/docs/definitions/index/functions/useAndroidDynamicColorCompat)
